...
OS は AI（Claude Code）× GitHub を前提に、速度と安全性を両立しながら学習も自然に生まれる**開発エコシステム**を提供する。以下の **Docs（ガイド）** と **Artifacts（実体）** で構成される。
...
```bash
# Instance の初期化（プレースホルダ置換・project_config.yml 生成・git hooks 設定）
./os_scripts/init \
...
git add -A
git commit -m "初期セットアップ: os_scripts/init による Instance 初期化"
...
### 3.1 `project_config.yml` — 設定の唯一の入口
...
### 3.2 `os_scripts/run` — 唯一の実行口

人も CI も `os_scripts/run` だけ叩けばよい。

| サブコマンド | 説明 |
|-------------|------|
| `./os_scripts/run format` | コードフォーマット |
| `./os_scripts/run lint` | リンター実行 |
| `./os_scripts/run typecheck` | 型チェック |
| `./os_scripts/run test` | テスト実行 |
| `./os_scripts/run build` | ビルド |
| `./os_scripts/run ci` | 標準 CI（lint → typecheck → test） |
| `./os_scripts/run doctor` | 設定不足・環境不足の診断 |
...
`os_scripts/hooks/pre-push` により、ローカルでの `git push` 時にも以下のチェックが自動で走る。

```bash
./os_scripts/run ci   # lint / typecheck / test を実行
```

巨大な差分（`project_config.yml` の `policy.max_diff_warning` / `policy.max_file_warning` を超過する変更）が含まれないかも併せてチェックされ、警告が表示される（push は継続される）。
...
- CI（GitHub Actions）が `./os_scripts/run doctor` → `./os_scripts/run ci` を自動実行
...
**出力例**：
```
[FAIL] project_config.yml が見つかりません
  → ./os_scripts/init を実行してください

[FAIL] commands.lint が未設定です
  → project_config.yml の commands.lint にリンターコマンドを設定してください

[FAIL] commands.test が未設定です
  → project_config.yml の commands.test にテストコマンドを設定してください
...
| 症状 | 原因 | 対処 |
|------|------|------|
| CI が動かない | `project_config.yml` が未生成 | `./os_scripts/init` を実行する |
| `lint` / `test` が失敗する | コマンドが未設定 or 空 | `project_config.yml` の `commands` を埋める |
...
| `os_docs/architecture.md` | アーキテクチャ・設計方針 |
